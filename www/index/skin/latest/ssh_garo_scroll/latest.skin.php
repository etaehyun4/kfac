...
$sliderwidth=378;// 스크롤러의 가로크기 
$sliderheight=105 ;// 스크롤러의 높이 ; 이미지높이보다 조금 길게
$img_width=110 ;// 이미지의 폭
...
$data_temp ="";
    ...
    $content = cut_str(get_text($list[$i][wr_content]),$img_height);
    $img = "$g4[path]/data/file/$bo_table/thumb/".urlencode($list[$i][file][0][file]);
/*	if(!$list[$i][file][0][file])
    $img = "$g4[path]/data/file/$bo_table/thumb/".urlencode($list[$i][file][1][file]);
	if(!$list[$i][file][1][file])
    $img = "$g4[path]/data/file/$bo_table/thumb/".urlencode($list[$i][file][2][file]);
*/
    if (!$list[$i][file][0][file] || !file_exists($img))
...
$data_temp .="<td style='padding-left:1pt;padding-right:1pt;'>";
$data_temp .="<TABLE cellSpacing=1 cellPadding=0  bgColor=#e0e0e0 border=0><TR><TD> <TABLE cellSpacing=1 cellPadding=0  bgColor=#f0f0f0 border=0><TR><TD align=middle bgColor=#ffffff >";  //테두리선

//실제내용시작
$data_temp .="<table border=0 cellpadding=0 cellspacing=0><tr><td  align=center ><a href='$g4[bbs_path]/board.php?bo_table=$bo_table&wr_id={$list[$i][wr_id]}'>{$img}</a></td></tr><tr><td height=5></td></tr><tr><td align=center><a href='$g4[bbs_path]/board.php?bo_table=$bo_table&wr_id={$list[$i][wr_id]}'>{$list[$i][subject]}</a> {$list[$i][icon_new]}</td></tr>";

//$data_temp .="<tr><td align=center><a href='$g4[bbs_path]/board.php?bo_table=$bo_table&wr_id={$list[$i][wr_id]}'><!--<font color=#FF3300>{$list[$i][ca_name]}</font>--></a> {$list[$i][icon_new]}</td></tr> ";
